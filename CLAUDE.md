...
- The header search box (`frontend/js/search.js`) selects a suburb via the same `AppState.select(id)` path as the sidebar's "similar suburb" buttons, and **`await`s `map.js`'s `focusSuburbAnywhere(suburb)` before calling `AppState.select()`, not concurrently with it** — `focusSuburbAnywhere()` resolves the suburb's council name to an id (`findCouncilByName()`, caches one `fetchCouncils()` call), `await`s `loadSuburbView()` to drill into it if it isn't already the council on screen, then delegates to `focusSuburb()` to pan/zoom/highlight the suburb — falling back to centering on its point `location` if the council can't be resolved, loading it fails, or the suburb has no boundary of its own. The `await`-before-`select()` ordering is load-bearing, not style: `loadSuburbView()` synchronously overwrites `#sidebar` with its own "now click a suburb" placeholder the moment it starts, and firing it concurrently with `AppState.select()`'s async detail fetch let that placeholder land *after* the real profile had already rendered, silently clobbering it back to the placeholder — sequencing them removes the race.
